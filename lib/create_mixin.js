...
function getProcessedStoreStateMapping() {
  ...
  Object.keys(storeStateMapping).forEach((storeName) => {
    const props = storeStateMapping[storeName];
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (!isArray && !isObject) {
        ...
        throw new Error(
          `[${this._floox.componentName}] Expected "${storeName}" property to be an array or an object`
        );
      }
    ...
    if (isArray) {
      ...
      Object.keys(props).forEach((key) => {
        addStoreProperty.call(this, mapping, store, storeName, props[key], key);
      });
    }
  });
